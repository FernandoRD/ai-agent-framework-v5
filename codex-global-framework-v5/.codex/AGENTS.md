...
Use the routing score and risk floors above to choose Luna, Terra, or Sol for
each unit independently, including discovery, implementation, and review.
...
- Select the smallest sufficient tier immediately. Do not try Luna first when
  ...
  affected unit when evidence shows the assigned capability is insufficient;
  keep remaining lower-tier work at its original tier.
...
- Model capability and independence are separate requirements: use the smallest
  reviewer meeting the review's risk floor, and preserve independent review
  even when the parent is capable of implementing the change.

### Publication uses the smallest capable agent

Commit, push, repository synchronization, and release preparation are separate
units of work. Route them independently from the implementation they publish.
Do not inherit the implementation's model tier merely because publication
happens in the same conversation or is invoked through a publishing skill.

- For routine, explicitly authorized publication with known destinations and
  validated changes, delegate the complete bounded workflow to `luna_worker`:
  inspect status and the scoped diff, preserve unrelated changes, stage explicit
  paths, create the requested commit, push the authorized branch to the authorized
  remotes, and verify remote hashes. Reuse an available Luna worker where possible.
- Pass a compact handoff with repository, branch, allowed files, destinations,
...
- Escalate only the affected unit when conflicts, uncertain scope, compatibility,
  release semantics, deployment, or other material risks require Terra or Sol.
  A routine Git commit/push does not by itself constitute a database migration or
  production infrastructure change. Deployment remains a separate risk assessment.
- Missing credentials, network access, or sandbox permission do not justify a
  stronger model. Request the required access through the normal approval path;
  never weaken permissions or bypass approval to keep work on a cheaper agent.
- Preserve all publication authorization and safety rules, including destination
  and privacy checks, explicit staging, no implicit force push or history rewrite,
  and verification of each remote. This routing policy grants no new authority to
  publish, deploy, create releases, or change repository visibility.
...
- For a large or unfamiliar repository, delegate one targeted read-only
  discovery task to `luna_explorer` before broad local exploration. Its capsule
  must contain relevant files and symbols, execution path, constraints, likely
  tests, and unresolved questions. Reuse it instead of repeating the scan.
- For an uncertain root cause, multiple components, coordinated multi-file
  ...
  with the parent. Examples: inspect collector/template contracts while the
  parent diagnoses Grafana, or update a bounded artifact while the parent
