...
Use the least expensive available model tier that can safely complete each
bounded unit.
...
- If the active model is below the required tier, delegate the affected unit to
  a subagent using the required tier.
- Never escalate the entire request when only one bounded unit requires Pro.
...
For a large or unfamiliar repository, use `flash-explorer` once for targeted,
